...
{
    public Add_beer_to_box_handler(Func<Guid, IEnumerable<Event>> event_stream, Action<Guid, Event> publish_event) :
        base(event_stream, publish_event)
    {
    }

    public override IEnumerable<Event> Handle_command(Box aggregate, Add_beer_to_box command)
    {
        try
        {
