...
{
    public Close_box_handler(Func<Guid, IEnumerable<Event>> event_stream, Action<Guid, Event> publish_event) : base(
        event_stream, publish_event)
    {
    }

    public override IEnumerable<Event> Handle_command(Box aggregate, Close_box command)
    {
